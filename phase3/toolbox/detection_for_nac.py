...
def check_arguments_errors(args):
	...
	if args.input and not os.path.exists(args.input):
		print('{}{}.IMG'.format(const.NAC_IMAGE_PATH, args.input))
...
def main():
	...
	nac = rg.read_nac('{}{}'.format(const.NAC_IMAGE_PATH, args.input))
	...
	for y in range(0, height, R-OVER):
		for x in range(0, width, R-OVER):
			...
			image, detections = image_detection(
				image_resized, darknet_image, network, class_names, class_colors, args.thresh
			)
			if len(detections) > 0:
				cv2.imwrite('{}{}_{}_{}result.jpg'.format(const.PHASE3_RESULT, args.input, x, y), image)
